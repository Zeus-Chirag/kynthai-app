...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-5 rounded-2xl border border-border/60 py-6 shadow-sm shadow-emerald-950/[0.03] dark:shadow-black/20",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-snug font-semibold tracking-tight text-[15px] sm:text-base", className)}
      {...props}
    />
  )
}
