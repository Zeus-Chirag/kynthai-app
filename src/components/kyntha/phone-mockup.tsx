...
export function PhoneMockup({ className, ariaHidden }: { className?: string; ariaHidden?: boolean }) {
  return (
    <motion.div
    ...
    >
      ...
      <div
        ...
        aria-hidden
        style={{ background: 'radial-gradient(ellipse closest-side, rgba(16,185,129,0.6), rgba(13,148,136,0.28) 45%, transparent 70%)' }}
      />
      <FloatingRing className="-inset-4 h-[110%] w-[110%]" delay={0.5} />
      <FloatingRing className="left-1/2 top-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2" delay={2.2} />
      ...
      >
        ...
        <div
          ...
          aria-hidden
          style={{ background: 'linear-gradient(130deg, rgba(255,255,255,0.15) 0%, transparent 55%)' }}
        />
        ...
        <div className="overflow-hidden rounded-[2.85rem] bg-white dark:bg-neutral-900">
          ...
          {/* Status bar */}
          <motion.div variants={ENTRANCE} custom={0} className="flex items-center justify-between px-5 pt-1.5 pb-0.5 text-[10px] font-semibold text-neutral-900 dark:text-neutral-100">
            <span>9:41</span>
          ...
          {/* App header */}
          <motion.div variants={ENTRANCE} custom={1} className="flex items-center justify-between px-4 pt-2 pb-1">
            <div className="flex items-center gap-2">
              <KynthaIcon size={22} />
              <span className="text-sm font-bold text-neutral-900 dark:text-neutral-100">Kyntha</span>
            </div>
          ...
          >
            ...
            <div className="mt-2 flex items-center gap-2.5">
              ...
              <motion.button whileTap={{ scale: 0.9 }} className="rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 px-3 py-1.5 text-[11px] font-bold text-white shadow-md shadow-emerald-700/30" type="button">
                Take
              </motion.button>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-[10px] font-bold text-amber-700 dark:text-amber-400">AI Insight · Kyntha Pro</p>
          ...
          <div className="mx-3 mb-2 mt-3 flex items-center justify-around rounded-2xl border border-neutral-200 bg-white/90 px-2 py-2 shadow-sm backdrop-blur dark:border-neutral-700 dark:bg-neutral-800/90 sm:mx-4 sm:px-4">
            ...
            <motion.div variants={TICK} initial="initial" animate="animate" className="-mt-5 flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-xl shadow-emerald-800/40 ring-4 ring-white dark:ring-neutral-900">
      ...
      <FloatingBadge className="-left-14 top-20 hidden md:flex" delay={1.1} icon={<Heart className="h-3.5 w-3.5 text-rose-500" />} title="Mom&apos;s BP taken" sub="By you · 2 min ago" />
      <FloatingBadge className="-right-12 top-40 hidden md:flex" delay={1.4} icon={<Search className="h-3.5 w-3.5 text-emerald-600" />} title="Drug interaction" sub="Metformin — safe" />
      <FloatingBadge className="-left-16 bottom-24 hidden lg:flex" delay={1.7} icon={<Sparkles className="h-3.5 w-3.5 text-amber-500" />} title="AI insight ready" sub="Weekly report" />
      <FloatingBadge className="-right-16 bottom-36 hidden lg:flex" delay={2} icon={<ShieldCheck className="h-3.5 w-3.5 text-blue-600" />} title="HIPAA-safe upload" sub="Lab report scanned" />
    </motion.div>
  )
}
...
function NavIcon({ icon, active }: { icon: React.ReactNode; active?: boolean }) {
  return (
    <motion.div whileTap={{ scale: 0.9 }} className={cn('flex h-7 w-7 items-center justify-center rounded-xl transition-colors', active ? 'bg-emerald-50 text-emerald-700' : 'text-neutral-400')}>
      {icon}
    </motion.div>
  )
}

function FloatingBadge({ className, icon, title, sub, delay = 0 }: { className?: string; icon: React.ReactNode; title: string; sub: string; delay?: number }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12, scale: 0.88 }}
      animate={{ opacity: 1, y: [0, -5, 0], scale: 1 }}
      transition={{ delay, duration: 0.6, ease: 'easeOut' }}
      className={cn('absolute z-20 flex items-center gap-2 rounded-2xl border border-neutral-200/80 bg-white/95 px-3 py-2 shadow-2xl shadow-emerald-900/20 backdrop-blur-xl', className)}
    >
