...
import { ReferralDashboard } from '@/components/kyntha/referral-dashboard'
import { HealthPulseRing } from '@/components/kyntha/health-pulse-ring'
...
export function ProfileHub({
  ...
  const switchablePortals = isAdmin
    ? [...publicPortals, 'admin']
    : publicPortals
  const switchLabel = switchablePortals
    .map((id) => {
      const map: Record<string, string> = { caretaker: 'Family', patient: 'Patient', doctor: 'Doctor', lab: 'Lab', admin: 'Admin' }
      return map[id]
    })
    .join(' · ')
  ...
  async function handleExportData(): Promise<void> {
    ...
    try {
      ...
      a.href = url
      a.download = `kyntha-data-export-${new Date().toISOString().slice(0, 10)}.json`
  ...
  return (
    <ResponsiveSheet open={open} onOpenChange={onOpenChange}>
        <SheetHeader className="px-5 pt-3 pb-3">
          <SheetTitle className={cn('text-sm text-muted-foreground', isMobile ? 'text-center' : 'text-left')}>
            Profile &amp; Settings
          </SheetTitle>
        </SheetHeader>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-semibold">
                  {scoreLoading ? 'Loading...' : healthScore !== null ? `${healthScore}/100` : 'No data yet'}
                </p>
          ...
          <Card>
            <CardContent className="p-0 divide-y divide-border/60">
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-destructive">{deleting ? 'Deleting…' : 'Delete my account'}</p>
                  <p className="text-xs text-muted-foreground">
              ...
              <DialogHeader>
                ...
                <DialogDescription asChild>
                  <div className="text-sm text-muted-foreground space-y-2">
                    ...
                    <p>This action <strong>cannot be undone</strong>. A 7-day cooldown applies before
...
const CONSENT_ITEMS: {
...
}[] = [
  {
    ...
    desc: 'Kyntha Terms & Privacy Policy',
    icon: Shield,
    tint: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400',
  },
  {
...
function ConsentManager({ consentFlags }: ConsentManagerProps) {
  ...
  }, [consentFlags.consentAccepted, consentFlags.dataProcessingConsent, consentFlags.aiTrainingConsent])
  ...
  async function handleToggle(key: keyof typeof localFlags, next: boolean) {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (data.consentAccepted !== undefined)       setLocalFlags((p) => ({ ...p, consentAccepted:       data.consentAccepted }))
      if (data.dataProcessingConsent !== undefined) setLocalFlags((p) => ({ ...p, dataProcessingConsent: data.dataProcessingConsent }))
      if (data.aiTrainingConsent !== undefined)     setLocalFlags((p) => ({ ...p, aiTrainingConsent:     data.aiTrainingConsent }))
    } catch {
