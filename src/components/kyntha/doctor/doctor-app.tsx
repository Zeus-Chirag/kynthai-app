...
export function DoctorApp({ user }: { user: AuthUser }) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">Preparing verification status and schedules…</p>
        </div>
      </div>
    )
  }
  ...
  if (state === 'pending') {
    return <PendingState user={user} onRefresh={load} onLogout={() => {
      router.push('/')
    }} />
  }
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-amber-50/50 via-background to-background dark:from-amber-950/20">
      <div className="mx-auto max-w-md px-4 py-12">
        <div className="flex flex-col items-center text-center">
          ...
          <Badge variant="secondary" className="mb-3 bg-amber-500/10 text-amber-600 dark:text-amber-400">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Thanks for submitting your details. Our admin team is reviewing your
            profile and documents. You&apos;ll receive an email once approved
