...
export function VideoCall({ roomName, displayName = 'Kyntha User', identity, onEndCall, role = 'patient' }: VideoCallProps) {
  ...
  const postSignal = React.useCallback(async (type: string, payload: Record<string, unknown>) => {
    try {
      await fetch('/api/webrtc-signaling', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ appointmentId: roomName, role, userId: identity || 'unknown', userName: displayName, type, payload }),
      })
    } catch (e) { console.error('[VideoCall]', e) }
  ...
  const buildPeerConnection = React.useCallback(() => {
    const pc = new RTCPeerConnection({
      iceServers: [
        ...
        ...(TURN_URL ? [{ urls: TURN_URL, username: TURN_USERNAME, credential: TURN_PASSWORD }] : []),
      ],
    })
  ...
  const handleEnd = React.useCallback(() => {
    ...
    try { pcRef.current?.close() } catch (e) { console.error('[VideoCall] close peerconnection', e) }
    stopAllTracks()
    setPeer({ connected: false })
    setJoining(true)
    setShowConsent(true)
  ...
  const poll = React.useCallback(async () => {
    ...
    try {
      ...
      const messages = (data as { messages?: Array<{ id?: string; type?: string; payload?: Record<string, unknown>; role?: string }> }).messages || []
      ...
      for (const msg of messages) {
        ...
        if (msg.type === 'offer' && msg.role !== role && pcRef.current) {
          ...
          for (const c of iceCandidatesBuffer.current) {
            try { await pc.addIceCandidate(c) } catch (e) { console.error('[VideoCall] addIceCandidate', e) }
          }
          ...
          await pcRef.current.setRemoteDescription(new RTCSessionDescription(msg.payload?.sdp as RTCSessionDescriptionInit))
          for (const c of iceCandidatesBuffer.current) {
            try { await pcRef.current!.addIceCandidate(c) } catch (e) { console.error('[VideoCall] addIceCandidate', e) }
          }
          iceCandidatesBuffer.current = []
        } else if (msg.type === 'ice-candidate' && pcRef.current) {
          try {
            await pcRef.current.addIceCandidate(new RTCIceCandidate(msg.payload?.candidate as RTCIceCandidateInit))
          } catch (e) { console.error("[VideoCall]", e) }
  ...
  React.useEffect(() => {
    ...
    async function init() {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 1280 }, height: { ideal: 720 }, facingMode: 'user' }, audio: { echoCancellation: true, noiseSuppression: true } })
        if (cancelled) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-black">
      <div className="relative flex-1 overflow-hidden bg-gradient-to-br from-emerald-950 via-black to-teal-950">
        ...
        {!peer.connected && !joining && !error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-center text-white">
            ...
            <p className="text-xs opacity-60">Room: <span className="font-mono">{roomName}</span></p>
          </div>
        )}
        ...
        {error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 px-6 text-center text-white">
            ...
            <Button onClick={handleEnd} variant="outline" className="mt-3 border-white/30 bg-white/10 text-white hover:bg-white/20">
              Close
            </Button>
          </div>
        )}
        ...
        <div className="absolute left-3 top-3 flex items-center gap-2">
          <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium backdrop-blur', peer.connected ? 'bg-emerald-500/20 text-emerald-200' : 'bg-white/10 text-white/80')}>
            <span className={cn('h-1.5 w-1.5 rounded-full', peer.connected ? 'bg-emerald-400' : 'bg-white/60')} />
      ...
      <div className="flex items-center justify-center gap-4 bg-black/90 px-6 py-5 backdrop-blur">
        <Button size="icon" variant="ghost" onClick={toggleMute} className={cn('h-12 w-12 rounded-full border border-white/10', muted ? 'bg-white/10 text-white' : 'bg-white/5 text-white hover:bg-white/10')} aria-label={muted ? 'Unmute' : 'Mute'} title={muted ? 'Unmute' : 'Mute'}>
          {muted ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
        </Button>

        <Button size="icon" variant="ghost" onClick={toggleVideo} className={cn('h-12 w-12 rounded-full border border-white/10', videoOn ? 'bg-white/5 text-white hover:bg-white/10' : 'bg-white/10 text-white')} aria-label={videoOn ? 'Turn off camera' : 'Turn on camera'} title={videoOn ? 'Turn off camera' : 'Turn on camera'}>
          {videoOn ? <Video className="h-5 w-5" /> : <VideoOff className="h-5 w-5" />}
        </Button>

        <Button size="icon" variant="ghost" onClick={handleEnd} className="h-12 w-12 rounded-full border border-red-500/40 bg-red-500/20 text-red-200 hover:bg-red-500/30" aria-label="End call" title="End call">
