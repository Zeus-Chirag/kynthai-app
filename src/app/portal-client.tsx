...
const PUBLIC_SCREENS  = ['landing', 'login', 'pricing', 'checkout', 'privacy', 'terms', 'cookies', 'accessibility', 'medical-disclaimer', 'refund-cancellation'] as const
...
export function PortalClient({ children }: { children: React.ReactNode }) {
  ...
  const isKnownPath = pathname === '/' || pathname in ROUTE_SCREEN || /^\/(patient|doctor|lab|caretaker|admin)$/.test(pathname)
  const router   = useRouter()
  const store    = useAppStore()
  const {
    user, screen, onboardingComplete, loginPortal,
    checkoutTier, checkoutFounder, currency,
    completeOnboarding, setLoginPortal,
  } = store
  ...
  const PUBLIC_PATHS = new Set(['/login', '/pricing', '/checkout', '/privacy', '/terms', '/cookies', '/accessibility', '/medical-disclaimer', '/refund-cancellation'])
  ...
  if (user && PORTAL_PATHS.has(pathname)) {
    ...
    if (pathname !== expectedPath && ['patient', 'doctor', 'lab', 'caretaker', 'family', 'admin'].includes(userPortal)) {
