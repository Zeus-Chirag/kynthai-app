...
import { requireAuth, requireAuthWithCsrf, jsonError, jsonPage, requireSystemToken, checkConsent, readJson, checkAiTier, validateBody, isResponseError, jsonOk } from '@/lib/api-helpers'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const isPersonalQuestion = /\b(my|I|me|myself|mine|should I|can I|why did|how do|what happens if|what should)\b/i.test(message) ||
                              /\b(side effect|reaction|allergic|swollen|nausea|dizzy|pain|feel|experiencing|started taking|on my)\b/i.test(message)
      ...
      try {
        await db.chatMessage.createMany({
          data: [
            ...
            { userId: u.id, role: 'assistant', content: dbReply, source: 'medicine-db', expiresAt: messageExpiry() },
          ],
        })
      } catch { /* ignore */ }
    ...
    if (!isAiAvailable()) {
      const msg = 'AI chat is not available yet. Set ZENMUX_API_KEY in your .env file to enable it. For now, you can search the medicine database directly.'
      try {
        await db.chatMessage.createMany({
          data: [
            { userId: u.id, role: 'user', content: message, expiresAt: messageExpiry() },
    ...
    if (u.allergies) {
      patientContextParts.push(`PATIENT ALLERGIES: ${u.allergies} — NEVER recommend medications containing these allergens.`)
    }
    ...
    const allCtxResults = await Promise.allSettled([
      ...
      db.medication.findMany({ where: { userId: u.id, active: true }, select: { name: true, dosage: true, frequency: true } }),
      // [PHI: CHRONIC CONDITIONS — name, severity]
      db.chronicCondition.findMany({ where: { patientId: u.id, active: true }, select: { name: true, severity: true } }),
      // [PHI: HEALTH JOURNAL — date, symptoms, mood, notes]
      db.healthJournal.findMany({ where: { userId: u.id }, orderBy: { date: 'desc' }, take: 3, select: { date: true, symptoms: true, mood: true, notes: true } }),
      // [PHI: CHAT HISTORY — role, content]
      db.chatMessage.findMany({ where: { userId: u.id }, orderBy: { createdAt: 'desc' }, take: 5, select: { role: true, content: true } }),
      // [PHI: EMERGENCY ALERTS — type, memberName, notes, tier]
      db.emergencyAlert.findMany({ where: { reporterId: u.id, status: 'active' }, orderBy: { createdAt: 'desc' }, take: 3, select: { type: true, memberName: true, notes: true, tier: true } }),
      ...
      (u.role === 'caretaker' || u.subscriptionTier === 'family_pro')
        ? db.familyHealthAlert.findMany({ where: { read: false }, orderBy: { createdAt: 'desc' }, take: 3, select: { type: true, title: true, message: true, severity: true } })
        : Promise.resolve([]),
      // [PHI: FAMILY MEMBERS — name, relation, conditions; caretaker/family_pro only]
      (u.role === 'caretaker' || u.subscriptionTier === 'family_pro')
        ? db.family.findMany({ where: { ownerId: u.id }, include: { members: { take: 5, select: { name: true, relation: true, conditions: true, role: true } } } })
        : Promise.resolve([]),
    ])
        ...
        .map((j: any) => {
          ...
          try {
            ...
            if (Array.isArray(symps) && symps.length > 0) {
              const names = symps.map((s: Record<string, unknown>) => typeof s.name === 'string' ? s.name : String(s)).join(', ')
    ...
    if (recentChats.length > 0) {
      const chatSummary = recentChats
        .reverse()
        .map((m: any) => `${m.role === 'user' ? 'Patient' : 'Dr. Kyntha'}: ${m.content.slice(0, 200)}`)
        .join('\n')
...
// [PHI SECTION] Active health alerts — pushed to AI prompt
    ...
    const allAlerts = [...(allCtx[4] ?? []), ...(allCtx[5] ?? [])] as Array<{ type?: string; memberName?: string; title?: string; message?: string; severity?: string; tier?: string; notes?: string }>
    if (allAlerts.length > 0) {
      const alertList = allAlerts
        .map((a) => {
          if (a.type && a.memberName) return `[${a.tier ?? a.severity}] ${a.type}: ${a.memberName} — ${a.notes ?? ''}`
...
// [PHI SECTION] Family members with conditions — pushed to AI prompt
    ...
    // NO TIMEOUT - execute directly, no restrictions
    // ──────────────────────────────────────────────────────────────────────────
    ...
    try {
      await db.chatMessage.createMany({
        data: [
          { userId: u.id, role: 'user', content: message, expiresAt: messageExpiry() },
          { userId: u.id, role: 'assistant', content: reply, source: 'llm', expiresAt: messageExpiry() },
        ],
      })
    } catch {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const qpResult = chatQuerySchema.safeParse(
