...
export async function POST(req: NextRequest) {
  try {
    ...
    // locked accounts and non-existent accounts must look identical.
    if (user.lockedUntil && user.lockedUntil > new Date()) {
      await logAudit(user.id, 'auth.login.locked', `lockedUntil=${user.lockedUntil.toISOString()}`);
    ...
    // HIPAA/compliance: enforce consent before issuing session — a user who
    // revoked consent must not get authenticated access to PHI endpoints.
