...
export async function GET(req: NextRequest) {
  ...
  await logAudit(sessionUser.id, 'health.pulse.read', { resourceType: 'HealthScore' })
    ...
    const [todayRems, yesterdayRems, streak, weekJournals, todayJournal, todayChatCount, yesterdayScore] = await Promise.all([
      medIds.length > 0
      ...
      db.chatMessage.count({ where: { userId, createdAt: { gte: new Date(new Date().setHours(0, 0, 0, 0)) } } }),
    ...
    const totalScore = Math.min(100, adherence + streakScore + journalScore + symptomScore + aiScore)
    ...
    const insight = generateInsight(totalScore, adherence, streakDays, weekJournals.length, todayJournal, todayChatCount, medications.length)

    // Persist score for trend tracking (non-blocking)
    const todayScore = totalScore
    const todayBreakdown = { adherence, streak: streakScore, journal: journalScore, symptoms: symptomScore, ai: aiScore }
    ...
    db.healthScore.upsert({
      where: { userId_date: { userId, date: todayDate } },
      create: { userId, date: todayDate, score: todayScore, breakdown: JSON.stringify(todayBreakdown) },
