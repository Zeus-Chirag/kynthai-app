  ...
  await logAudit(sessionUser.id, 'consultation.prep', { resourceType: 'ConsultationNote' })
    ...
    if (isDemo) {
      return jsonOk({
        ...
        symptoms: [
          { date: new Date(Date.now() - 86400000).toISOString(), symptoms: ['Mild headache', 'Fatigue'], severity: 2 },
        ],
    ...
    const [medications, conditions, user] = await Promise.all([
      db.medication.findMany({ where: { userId, active: true }, select: { id: true, name: true, dosage: true, frequency: true } }),
      db.chronicCondition.findMany({ where: { patientId: userId, active: true }, select: { name: true } }),
      db.user.findUnique({ where: { id: userId }, select: { allergies: true } }),
    ])

    const medIds = medications.map((m) => m.id)
    const reminders = medIds.length > 0
      ? await db.reminder.findMany({ where: { medicationId: { in: medIds } }, select: { status: true } })
      : []
    const totalDoses = reminders.length
    const taken = reminders.filter((r) => r.status === 'taken').length
    const adherence = { totalDoses, taken, percentage: totalDoses > 0 ? Math.round((taken / totalDoses) * 100) : 0 }
