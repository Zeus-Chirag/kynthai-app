...
function getVideoTokenSecret(): string {
  ...
  if (!secret) {
    throw new Error('VIDEO_TOKEN_SECRET env var is required for video-call token generation')
  }
  return secret
}

export function verifyVideoToken(token: string): { uid: string; name: string; room: string; exp: number } | null {
  const secret = process.env.VIDEO_TOKEN_SECRET
...
export async function POST(req: NextRequest) {
  ...
  let secret: string
  try {
    secret = getVideoTokenSecret()
  } catch (err) {
    return jsonError('Server misconfiguration: video token secret not set', 500)
  }

  const body = await readJson<{ appointmentId?: string; roomName?: string }>(req)
  if (!body) return jsonError('Invalid JSON', 400)

  const roomName = sanitizeText(body.roomName, 80) || `kyntha-${body.appointmentId || crypto.randomUUID().slice(0, 8)}`
