...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'invite') {
    ...
    if (!body.email || !body.name || !body.relation) return jsonError('Email, name, relation required', 400)
    const email = sanitizeText(body.email, 254).toLowerCase()
    if (!isValidEmail(email)) return jsonError('Valid email required', 400)

    // Guard against inviting yourself
    if (email === u.email) {
      return jsonError('You cannot invite yourself to your own family.', 400, 'SELF_INVITE_FORBIDDEN')
    }
    ...
    if (!family) family = await db.family.create({ data: { name: `${u.name}'s Family`, ownerId: u.id } })
    ...
    const existing = await db.familyMember.findFirst({ where: { familyId: family.id, inviteEmail: email } })
  ...
  if (body.action === 'accept') {
    ...
    if (body.relationVerificationToken) {
      const tokenMatches =
        pending.inviteToken === body.relationVerificationToken ||
        pending.relationVerificationToken === body.relationVerificationToken
      if (!tokenMatches) {
        return jsonError(
    ...
    await db.familyMember.update({
      where: { id: pending.id },
      data: { userId: u.id, inviteStatus: 'accepted', inviteToken: null, relationVerificationToken: null },
    })
...
export async function GET(req: NextRequest) {
  ...
  return jsonOk(pending.map(m => ({
    id: m.id, familyId: m.familyId,
    invitedBy: m.family.owner.name, invitedByEmail: m.family.owner.email,
    name: m.name, relation: m.relation, familyName: m.family.name,
  })))
}
