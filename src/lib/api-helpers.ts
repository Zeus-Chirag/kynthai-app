...
export const ADMIN_EMAILS = rawAdminEmails
  ...
  : process.env.NODE_ENV === 'production' ? [] : ['admin@demo.kyntha.app', 'admin@demo.com']
...
export function applyStandardHeaders(
  ...
  const finalRequestId = incomingRequestId ?? (() => {
    try {
      // @ts-ignore
...
export function jsonPage<T>(items: T[], opts: { cursor?: string | null; limit: number; hasMore: boolean; totalCount?: number }): NextResponse {
...
export async function requireAuthWithVerified(
  ...
  if (!r.user.emailVerified) {
    return { response: jsonError('Email verification required. Please verify your email first.', 403), user: null }
  }
  return r
}
...
export async function requireSystemToken(req: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'production') {
    return { response: jsonError('Forbidden — CRON_SECRET is required in production', 403), user: null }
  }
  if (!cronSecret || process.env.ALLOW_ADMIN_AS_SYSTEM !== 'true') {
    return { response: jsonError('Forbidden — system token required (enable ALLOW_ADMIN_AS_SYSTEM in dev)', 403), user: null }
  }
  const r = await requireAuth(req)
  if (r.response || !r.user) return r
  if (r.user.role !== 'admin' && !ADMIN_EMAILS.includes(r.user.email)) {
    return { response: jsonError('Forbidden — system token or admin role required', 403), user: null }
  }
  return r
}
...
export function checkConsent(
  user: { consentAccepted: boolean; dataProcessingConsent: boolean; aiTrainingConsent: boolean; isDemo?: boolean },
  req?: NextRequest
): NextResponse | null {
  ...
  if (!user.consentAccepted) {
    return applyStandardHeaders(NextResponse.json({ error: 'You must accept the Terms of Service to use this feature.' }, { status: 403 }))
  }
  if (!user.dataProcessingConsent) {
    return applyStandardHeaders(NextResponse.json({ error: 'You must consent to data processing to use health features.' }, { status: 403 }))
  }
  if (!user.aiTrainingConsent) {
    return applyStandardHeaders(NextResponse.json({ error: 'You must consent to AI data processing to use AI-powered features.' }, { status: 403 }))
  }
  return null
}
 ...
 */
export async function checkAiTier(
  user: User,
  featureName: string,
): Promise<NextResponse | null> {
  const tier = user.subscriptionTier
  if (tier === 'plus' || tier === 'family_pro') return null // unlimited
...
export async function validateBody<T extends z.ZodTypeAny>(
  ...
  if (!result.success) {
    ...
    return jsonError(
      ...
      422,
      'VALIDATION_ERROR',
      { fields: fieldErrors, issues: result.error.issues.map(i => ({ path: i.path, message: i.message })) }
    )
  }
  return result.data
}
