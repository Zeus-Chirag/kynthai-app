...
export async function verifySessionToken(signed: string): Promise<string | null> {
  ...
  // S3: session revocation check — only on Node (not Edge). Edge uses Supabase JWT, not local fallback cookie.
  // Guard the db import so Edge builds don't bundle Node's process.on.
  if (typeof process !== 'undefined' && process.env.NEXT_RUNTIME !== 'edge') {
    try {
      const { db } = await import('@/lib/db');
      const revoked = await db.revokedSession.findUnique({ where: { sessionJti: providedHmac } });
