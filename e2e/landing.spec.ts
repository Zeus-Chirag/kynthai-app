...
test.describe('Landing Page', () => {
  ...
  test('hero section has headline and description', async ({ page }) => {
    ...
    // Phone mockup should be visible
    const phoneMockups = page.getByTestId('phone-mockup');
    if (await phoneMockups.count() > 0) {
      await expect(phoneMockups.first()).toBeVisible();
    }
  });
